...
nng_http_get_status - return HTTP status code
...
uint16_t nng_http_get_status(const nng_http *conn);
----

== DESCRIPTION

The `nng_http_get_status()` returns a numeric code corresponding to
the HTTP status of the most recent (or next) exchange on _conn_.
...
xref:nng_http_get_reason.3http.adoc[`nng_http_get_reason()`].
