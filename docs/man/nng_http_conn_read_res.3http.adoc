...
void nng_http_conn_read_res(nng_http_conn *conn, nng_aio *aio);
----
...
xref:nng_http_conn_transact.3http.adoc[`nng_http_conn_transact()`] function,
which provides a simpler interface for performing a complete HTTP client
transaction.
